...
class AbandonConflictTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Verify that B and C's 10 ROBEE outputs are available for spending again because AB1 is now conflicted
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance + Decimal("20"))
        balance = newbalance

        # There is currently a minor bug around this and so this test doesn't work.  See Issue #7315
        # Invalidate the block with the double spend and B's 10 ROBEE output should no longer be available
